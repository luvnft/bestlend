...
use pyth_sdk_solana::state::{
    load_price_account, AccountType, PriceAccount, PriceStatus, PriceType, Rational, MAGIC,
    VERSION_2,
};

declare_id!("6Lh68KvhjwFGUENYh97S5BdLubB91bEcgKc2BuU2M13q");
...
pub mod mock_pyth {
    ...
    pub fn write_pyth_price(
        ...
        let mut def = PriceAccount::default();
        let exist_price_data = load_price_account(&account_data).unwrap_or(&mut def);

        let mut price_data: PriceAccount = unsafe { std::mem::zeroed() };
        ...
        account_data.copy_from_slice(unsafe {
            &std::mem::transmute::<PriceAccount, [u8; std::mem::size_of::<PriceAccount>()]>(
                price_data,
            )
        });

        // check price feed is valid
        let price_account = load_price_account(account_data).map_err(|e| {
...
pub struct Write<'info> {
    ...
    #[account(init_if_needed, payer = signer, space = std::mem::size_of::<PriceAccount>())]
